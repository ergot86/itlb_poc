...
;fill mem where large page points with 0xcc (int3), so it faults when executed
	mov 	rdi, CODE_ADDR
	mov 	rcx, 0x1000
	mov 	al,0xcc ;int3
	rep 	stosb

;mark the adress we'll be reading with 0xf1 (int1, also faults)
	mov [CODE_ADDR + CODE_OFFSET + 1],dword 0xf1f1f1f1
...
;tlb flush can be added to demonstrate that this is really a TLB issue
;(causes code from large page to be executed instead)

;	mov 	eax,0x90d8200f ; mov rax,cr3
;	stosd
;	mov 	eax,0x90d8220f ; mov cr3,rax
;	stosd

;read from the code to demonstrate iTLB/dTLD desync
	mov 	eax,0x24248b4d ; mov r12,[r12]
	stosd

;finally, a RET at end of code
	mov 	al,0xc3 ;ret
	stosb
...
;set up addr to be read by code in 4k page
;this will be 'mov r12,[r12]' in 4k page and 'int1;int1;int1;int1' in the large page
	lea 	r12,[r15 + CODE_OFFSET]
